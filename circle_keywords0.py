...
#载入语料
circle_topic = open(u"E:/pycharm_project/important_msg/circle/circle_topic111.txt")
#circle_topic = open(u"E:/pycharm_project/important_msg/circle/circle_topic1test.txt")
cirtop = circle_topic.readlines()
cirtop[0] = cirtop[0].replace('\xef\xbb\xbf', '')

#写入合并同一个用户，同一个话题的消息内容。写入内容和关键词
join_user_topic = open(u"E:/pycharm_project/important_msg/circle/join_user_topic_test1_test10.txt",'w')
keywords_user_topic = open(u"E:/pycharm_project/important_msg/circle/keywords_user_topic_test10.txt",'w')
usr_list = []
...
while usr_list:
    ...
    for k in range(len(usr_list)):
        if newlist[i][0] == usr_list[k][0]:
            newlist[i][2] = newlist[i][2] + usr_list[k][2]+newlist[i][0]
            #del usr_list[k]
            #print k
            index_list.append(k)
    #print index_list
    ...
    print newlist[i][0]
    #join_user_topic.write(u"{}\t{}\r\n".format(newlist[i][0], newlist[i][2]))

    keywords_user_topic.write(u"{}\t".format(newlist[i][0]))
    # content_list = i.replace('#', '').replace('一一', '').strip('\n').split('\t')
    newlist[i][2] = ''.join(x for x in unicode(newlist[i][2]) if ord(x) >= 256)
    newlist[i][2] = newlist[i][2].replace('艾艾贴','')
    #print 'zzz', newlist[i][2].replace('艾艾贴','')
    key_content = jieba.analyse.extract_tags(newlist[i][2], topK=15, withWeight=False)
    #print key_content
